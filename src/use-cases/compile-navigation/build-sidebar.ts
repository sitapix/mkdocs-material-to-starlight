...
export async function buildSidebar(
  ...
  const hasAuthoredNavigation =
    literateNav.tree !== null || (input.nav !== null && input.nav.length > 0);
  const compiledEntries = hasAuthoredNavigation
    ? sidebarResult.value.entries
    : compileAutogeneratedNavigation(input.slugMap);
  const filteredEntries = filterSidebarSlugs(compiledEntries, droppedBlogSlugs);
