...
describe('serializePackageJson', () => {
  ...
  it('pins @astrojs/starlight to ^0.40.0 or newer (sidebar autogen items shape, 0.39.1 icons, github-alerts 0.3.0 peer)', () => {
    ...
    //
    // Floor raised to 0.40.0: the github-alerts feature now pins
    // starlight-github-alerts@^0.3.0, which peers `@astrojs/starlight
    // >=0.40.0`. A Starlight pin <0.40.0 would make that feature
    // unresolvable (ERESOLVE). Note we stay on Astro 6 — no Starlight
    // release supports Astro 7 yet (0.40.0 peers `astro@^6.4.5`).
    const out = serializePackageJson({ siteName: 'X', siteDescription: null });
    const version = JSON.parse(out).dependencies['@astrojs/starlight'];
    const match = version.match(/^\^?(\d+)\.(\d+)\.(\d+)/);
    expect(match).not.toBeNull();
    const [major, minor] = [Number(match[1]), Number(match[2])];
    const meetsFloor = major > 0 || (major === 0 && minor >= 40);
    expect(meetsFloor, `expected >=0.40.0, got ${version}`).toBe(true);
  });
});
