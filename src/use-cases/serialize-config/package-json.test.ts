...
describe('serializePackageJson', () => {
  ...
  it('does NOT bundle starlight-md-txt (0.1.0 crashes on HTML comments like <!-- more -->)', () => {
    const out = serializePackageJson({ siteName: 'X', siteDescription: null });
  ...
  it('ships @astrojs/check and typescript as devDependencies so astro check works on first install', () => {
    // Without these, `astro check` renders an interactive install prompt —
    // which hangs for the full timeout under the converter's non-interactive
    // `--check` pass (field-tested 2026-07-23).
    const out = serializePackageJson({ siteName: 'X', siteDescription: null });
    const dev = JSON.parse(out).devDependencies;
    expect(dev).toHaveProperty('@astrojs/check');
    expect(dev).toHaveProperty('typescript');
    // @astrojs/check 0.9.x peers `typescript ^5 || ^6`; a ^7 pin would
    // ERESOLVE the fresh install this exists to fix.
    expect(dev.typescript.startsWith('^7')).toBe(false);
  });

  it('pins @astrojs/starlight to ^0.41.0 or newer (Astro 7 stack, companion-plugin peer floors)', () => {
    // Starlight 0.41.0 is the Astro 7 release (peers `astro@^7.0.2`,
    // drops Astro 6). Our core pin rides `astro@^7`, and several
    // companion plugins at their current pins peer `@astrojs/starlight
    // >=0.41.0` (links-validator 0.25, blog 0.28, github-alerts 0.4,
    // heading-badges 0.8, image-zoom 0.15, llms-txt 0.11). A Starlight
    // pin <0.41.0 would make the astro pin unresolvable AND those
    // features ERESOLVE on a fresh install.
    //
    // (Older floors, still binding: 0.39.0 reshaped autogenerated sidebar
    // groups to the `items: [{ autogenerate }]` form our serializer
    // emits; 0.39.1 shipped 13 built-in icons the curated icon map
    // references.)
    const out = serializePackageJson({ siteName: 'X', siteDescription: null });
    ...
    const meetsFloor = major > 0 || (major === 0 && minor >= 41);
    expect(meetsFloor, `expected >=0.41.0, got ${version}`).toBe(true);
  });
});
