...
describe('serializeSidebar', () => {
  ...
  it('serializes an autogenerate group with directory using the iterable-items shape', () => {
    // Starlight 0.36+ rejects the legacy `{ label, autogenerate: {...} }`
    // form ("Sidebar entries with `autogenerate` must be wrapped in an
    // `items` array") — wrap the autogenerate descriptor in `items`.
    const entries: ReadonlyArray<SidebarEntry> = [
      { kind: 'auto', label: 'Reference', directory: 'reference' },
    ];
    const out = serializeSidebar(entries);
    expect(out).toContain(`label: 'Reference'`);
    expect(out).toContain(`items: [{ autogenerate: { directory: 'reference' } }]`);
  });

  it('serializes an autogenerate group with collapsed: true on both the outer group and the autogenerate descriptor', () => {
    // Starlight 0.36+ no longer cascades `collapsed` from a parent group into
    // its autogenerated subgroups. Setting both keeps the prior behavior
    // (collapse the wrapper *and* every subgroup beneath it).
    const entries: ReadonlyArray<SidebarEntry> = [
      { kind: 'auto', label: 'Reference', directory: 'reference', collapsed: true },
    ];
    const out = serializeSidebar(entries);
