...
function renderAuto(entry: AutoEntry): string {
  // Starlight 0.36+ requires autogenerate descriptors to live inside an
  // `items` array, and no longer cascades the parent group's `collapsed`
  // into autogenerated subgroups. To preserve "collapse the whole subtree"
  // semantics for AutoEntry.collapsed, set `collapsed` on both the outer
  // group and the autogenerate descriptor.
  const outerCollapsed = entry.collapsed === true ? ', collapsed: true' : '';
  const innerCollapsed = entry.collapsed === true ? ', collapsed: true' : '';
  const inner = `{ autogenerate: { directory: ${quote(entry.directory)}${innerCollapsed} } }`;
  return `{ label: ${quote(entry.label)}${outerCollapsed}, items: [${inner}] }`;
}
