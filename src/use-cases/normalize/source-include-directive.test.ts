...
import { normalizeSourceIncludeDirectives } from './source-include-directive.js';

describe('normalizeSourceIncludeDirectives', () => {
  it('passes through text with no snippet directives', () => {
    const input = 'Just a paragraph.\n\n## Heading\n\nMore text.\n';
    const result = normalizeSourceIncludeDirectives(input);
  ...
  it('replaces a {* path *} line with an HTML TODO comment', () => {
    ...
    const result = normalizeSourceIncludeDirectives(input);
    expect(result.text).toMatch(/TODO.*source-include directive/);
    expect(result.text).toMatch(/docs_src\/first_steps\/tutorial001\.py/);
    expect(result.text).not.toMatch(/```text/);
    expect(result.diagnostics).toHaveLength(1);
    expect(result.diagnostics[0]?.ruleId).toBe('source-include-directive-detected');
  });
  ...
  it('is idempotent', () => {
    ...
    const once = normalizeSourceIncludeDirectives(input);
    const twice = normalizeSourceIncludeDirectives(once.text);
