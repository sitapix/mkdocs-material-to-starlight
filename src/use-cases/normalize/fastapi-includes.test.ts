...
describe('normalizeFastapiIncludes', () => {
  it('passes through text containing no fastapi includes', () => {
    const input = 'Just a paragraph.\n\n## Heading\n\nMore text.\n';
    expect(normalizeFastapiIncludes(input)).toBe(input);
  });

  it('wraps a `{* path *}` line in a fenced code block', () => {
    // fastapi uses `{* ../../docs_src/foo.py *}` to inline source from a
    ...
    const output = normalizeFastapiIncludes(input);
  ...
  it('does not match inline `{*...*}` in the middle of a paragraph', () => {
    // The marker is a block-level construct in fastapi — it stands alone on
    // a line. A `{* something *}` inside prose is something else and should
    // be left alone.
  ...
  it('is idempotent — wrapping is a no-op the second time', () => {
    ...
    const once = normalizeFastapiIncludes(input);
    const twice = normalizeFastapiIncludes(once);
